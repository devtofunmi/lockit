...
async function encryptMessage(plainText: string): Promise<{ encryptedContent: string }> {
...
const MessageCreated: React.FC<MessageCreatedProps> = ({ messageId, onCopied }) => {
  return (
    <div>
      <p>Message ID: {messageId}</p>
      <button onClick={onCopied}>Copy</button>
    </div>
  );
};
...
const MessageForm: React.FC<MessageFormProps> = ({ onNewMessage }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { encryptedContent } = await encryptMessage(messageContent.trim());
  ...
  if (createdId) {
    return (
      <>
        ...
        <MessageCreated
          messageId={createdId}
          onCopied={() => setCreatedId(null)} encryptionKey={''}        />
      </>
    );
  }
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="expiration" className="block text-gray-700 mb-1">
        ...
        <div className="flex items-center justify-between">
          <span className="text-gray-700">Enable Self-Destruct after viewing</span>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
